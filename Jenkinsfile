...
pipeline {
	...
	stages {
		...
		stage("sqi_deploy") {
			steps{
				script{
					withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${sqi_onprim_credentials_id}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
						withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${docker_credentials_id}", usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS']]) {
							sh"rm -rf deploy_dev.sh && echo "docker login ${docker_registry} -u $DOCKER_USER -p $DOCKER_PASS" >> deploy_dev.sh"
							sh"echo "docker stop ${service_image}" >> deploy_dev.sh"
							sh"echo "docker rm -f ${service_image}" >> deploy_dev.sh"
							...
							sh"set +x && echo "docker run -d -t -i $DOCKER_ENV -p ${service_port}:${container_port} --name=${service_image} ${docker_registry}/${service_image}:${SERVICE_TAG}" >> deploy_dev.sh"
							sh "cat deploy_dev.sh | sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $USERNAME@${sqi_onprim_server}"
							sh"rm -rf deploy_dev.sh"
						}
					}
				}
		...
		stage("preprod_deploy") {
			steps{
				script{
					withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${preprod_onprim_credentials_id}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
						withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${docker_credentials_id}", usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS']]) {
							sh"rm -rf deploy_dev.sh && echo "docker login ${docker_registry} -u $DOCKER_USER -p $DOCKER_PASS" >> deploy_dev.sh"
							sh"echo "docker stop ${service_image}" >> deploy_dev.sh"
							sh"echo "docker rm -f ${service_image}" >> deploy_dev.sh"
							...
							sh"set +x && echo "docker run -d -t -i $DOCKER_ENV -p ${service_port}:${container_port} --name=${service_image} ${docker_registry}/${service_image}:${SERVICE_TAG}" >> deploy_dev.sh"
							sh "cat deploy_dev.sh | sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $USERNAME@${preprod_onprim_server}"
							sh"rm -rf deploy_dev.sh"
						}
					}
				}
		...
		stage("prod_deploy") {
			steps{
				script{
					withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${prod_onprim_credentials_id}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
						withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: "${docker_credentials_id}", usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS']]) {
							sh"rm -rf deploy_dev.sh && echo "docker login ${docker_registry} -u $DOCKER_USER -p $DOCKER_PASS" >> deploy_dev.sh"
							sh"echo "docker stop ${service_image}" >> deploy_dev.sh"
							sh"echo "docker rm -f ${service_image}" >> deploy_dev.sh"
							...
							sh"set +x && echo "docker run -d -t -i $DOCKER_ENV -p ${service_port}:${container_port} --name=${service_image} ${docker_registry}/${service_image}:${SERVICE_TAG}" >> deploy_dev.sh"
							sh "cat deploy_dev.sh | sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $USERNAME@${prod_onprim_server}"
							sh"rm -rf deploy_dev.sh"
						}
					}
				}
